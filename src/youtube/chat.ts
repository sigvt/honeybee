...
export function getContinuation(
  ...
  if (
    ...
    log("only playerSeekContinuationData");
    return undefined;
  }
...
function parseChatAction(action: RawAction): Action | undefined {
  ...
  switch (type) {
        ...
        for (const badge of renderer.authorBadges) {
          ...
          switch (iconType) {
            ...
            case undefined:
              ...
              break;
            default:
              log("Unrecognized iconType: " + iconType);
              process.exit(1);
          }
        }
      }
    ...
    case "addBannerToLiveChatCommand":
      ...
      break;
    default:
      log("Unsupported actionType: " + type);
      appendJSON(join(process.cwd(), "unrecognizedActions.jsonl"), action);
  }
}
